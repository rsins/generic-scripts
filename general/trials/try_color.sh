...
myprint() 
{
   ...
   then
         ...
         tput rmso
      	 ...
      	 printf "${EINS}${TEXT_BG_FG_Format}${PrintText}\033[0m"
      fi
   else
      printf "$PrintText"
   fi
...
drawRectangle() {
   ...
   myprint "" $lowX $lowY `expr $maxX - $lowX ` $bold y " " "$text_format"
   myprint "" $lowX $maxY `expr $maxX - $lowX ` $bold y " " "$text_format"
   j=$lowY
   while test $j -le $maxY
   do
      myprint "" $lowX $j 1 $bold y " " "$text_format"
      myprint "" $maxX $j 1 $bold y " " "$text_format"
      j=`expr $j + 1`
   done
...
try2() {
   ...
   do
	 ...
	 "q")
      ...
      drawRectangle $x1 $x2 $y1 $y2 y $TEXT_FORMAT
      read -n 1 -s key
   done
}
